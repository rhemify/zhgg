...
describe('applyStep — orchestrator step → TUI state', () => {
  it('payment.request brings audit + oracle online', () => {
    ...
    expect(s.agents.get('audit.zhgg.eth')?.status).toBe('running');
    expect(s.agents.get('oracle.zhgg.eth')?.status).toBe('running');
  });
  ...
  it('payment.settle pushes a split row and updates oracle action', () => {
    ...
    expect(s.agents.get('oracle.zhgg.eth')?.lastAction).toMatch(/payment settled/);
  });
  ...
  it('audit.complete with compliant verdict marks audit done', () => {
    ...
    expect(s.agents.get('audit.zhgg.eth')?.status).toBe('done');
    expect(s.agents.get('audit.zhgg.eth')?.lastAction).toContain('compliant');
  });
  ...
  it('audit.complete with non-compliant verdict marks audit error', () => {
    ...
    expect(s.agents.get('audit.zhgg.eth')?.status).toBe('error');
  });
  ...
  it('spend_cap.exceeded marks audit error with reason', () => {
    ...
    const audit = s.agents.get('audit.zhgg.eth');
  ...
  it('axiom + memory_root events update audit lastAction without changing status', () => {
    ...
    expect(s.agents.get('audit.zhgg.eth')?.lastAction).toContain('AXIOM');
    expect(s.agents.get('audit.zhgg.eth')?.status).toBe('running');

    applyStep(s, { tMs: 200, name: 'audit.memory_root.pin' });
    expect(s.agents.get('audit.zhgg.eth')?.lastAction).toContain('memoryRoot');

    applyStep(s, { tMs: 300, name: 'audit.axiom.reveal' });
    expect(s.agents.get('audit.zhgg.eth')?.lastAction).toContain('revealed');
  });
